...
public class ArvoreB {
	...
	public void inserir(short chave, long endereco) throws Exception {
		indexFile.seek(0);
		KeyAdressPair aux = inserir(chave, endereco, indexFile.readLong());
	...
	private KeyAdressPair inserir(short chave, long endereco, long endPag) throws Exception {
		byte[] baPagAtual = new byte[(18 * ordem) - 8];
		...
		KeyAdressPair pair = new KeyAdressPair();
		...
		if(!paginaAtual.isLeaf()) {
			...
			pair = new KeyAdressPair(chave, endereco);
		}
		...
		if(paginaAtual.getN() < this.ordem - 1) {
			...
			indexFile.seek(endPag);
			indexFile.write(paginaAtual.getBa());
			return null;
			...
			KeyAdressPair intermediario = new KeyAdressPair();
			...
			if(pair.getKey() > paginaAtual.getPairAtIndexOf(paginaAtual.getN() - 1).getKey() && pair.getKey() < paginaIrma.getPairAtIndexOf(0).getKey()) {
				...
				intermediario.setAdress(pair.getAdress());
			}
			...
			indexFile.write(paginaIrma.getBa());
	...
	private void inserirOrdenado(KeyAdressPair pair, Pagina pagina) throws Exception {
		int i = 0;
	...
	public long search(short key) throws Exception {
		//chama o método recursivo passando como endereço a raiz
		indexFile.seek(0);
		KeyAdressPair resp = search(key, indexFile.readLong());
		if(resp != null) {
			return resp.getAdress();
		}
		else {
			return -1;
		}
	...
	private KeyAdressPair search(short key, long endPag) throws Exception {
		byte[] baPagAtual = new byte[(18 * ordem) - 8];
		indexFile.seek(endPag);
		indexFile.read(baPagAtual);
		Pagina paginaAtual = new Pagina(baPagAtual, this.ordem);
		KeyAdressPair aux = new KeyAdressPair();
	...
	public void updateAdress(short key, long newAdress) throws Exception {
		//chama o método recursivo passando como endereço a raiz
		indexFile.seek(0);
		update(indexFile.readLong(), key, newAdress);
	}

	private void update(long endPag, short key, long newAdress) throws Exception {
		byte[] baPagAtual = new byte[(18 * ordem) - 8];
		indexFile.seek(endPag);
		indexFile.read(baPagAtual);
		Pagina paginaAtual = new Pagina(baPagAtual, this.ordem);
		...
		if(!paginaAtual.isLeaf()) {
			...
			for(int i=0; i<paginaAtual.getN(); i++) {
				...
				if(key == aux.getKey()) {
					...
					aux.setAdress(newAdress);
					paginaAtual.setPairAtIndexOf(aux, i);
					indexFile.seek(endPag);
					indexFile.write(paginaAtual.getBa());
				}
				else if(key < aux.getKey()) {
					update(paginaAtual.getSonAtIndexOf(i), key, newAdress);
				}
			}
			//Se nenhuma das verificações anteriores forem verdadeiras, a única possibilidade restante é que a chave procurada é maior que a última chave 
			update(aux.getRightSon(), key, newAdress);
		}
		else {
	...
	public long remove(short key) throws Exception {
		indexFile.seek(0);
		return remove(key, indexFile.readLong(), -1);
		//Chamada para a função passando a raiz como endereço
	}

	private long remove(short key, long endPagAtual, long endPagPai) throws Exception {
		...
		if(!pagAtual.isLeaf()) {
			...
			for(int i=0; i<pagAtual.getN(); i++) {
				...
				if(key == aux.getKey()) {
					resp = aux.getAdress();
					KeyAdressPair antecessor = getAntecessor(pagAtual, (short)i);
					...
					aux.setAdress(antecessor.getAdress());
					...
					indexFile.seek(endPagAtual);
					indexFile.write(pagAtual.getBa());
			...
			while(bottom <= top) {
				middle = (bottom + top)/2;
				short midElement = pagAtual.getPairAtIndexOf(middle).getKey();
				if(midElement == key) {
					resp = pagAtual.getPairAtIndexOf(middle).getAdress();
					exists = true;
					break;
				}
			...
			if(!exists) {
				...
				if(pagAtual.getN() > this.ordem / 2) {
					...
					byte[] baPagPai = new byte[(18 * ordem) - 8];
					...
					if(siblingIndex == 1) {
						...
						KeyAdressPair aux = pagPai.getPairAtIndexOf(siblingIndex);
						aux.setLeftSon(-1);
						aux.setRightSon(-1);
						...
						KeyAdressPair aux = pagPai.getPairAtIndexOf(siblingIndex - 1);
						aux.setLeftSon(-1);
						aux.setRightSon(-1);
						//aux recebe o par chave/endereço que será inserido na pagAtual, e será substituido na pagPai
					...
					byte[] baPagPai = new byte[(18 * ordem) - 8];
					byte[] baPagIrma = new byte[(18 * ordem) - 8];
					indexFile.seek(endPagPai);
					indexFile.read(baPagPai);
					Pagina pagPai = new Pagina(baPagPai, this.ordem);
					...
					if(index < pagPai.getN()) {
						...
						KeyAdressPair aux = pagPai.getPairAtIndexOf(index);
						aux.setLeftSon(-1);
						aux.setRightSon(-1);
						inserirOrdenado(aux, pagAtual);
						...
						KeyAdressPair aux = pagPai.getPairAtIndexOf(index - 1);
						aux.setLeftSon(-1);
						aux.setRightSon(-1);
	...
	private KeyAdressPair stealShortest(long pagAdress) throws Exception{
		byte[] baPagAtual = new byte[(18 * ordem) - 8];
		indexFile.seek(pagAdress);
		indexFile.read(baPagAtual);
		Pagina pagAtual = new Pagina(baPagAtual, this.ordem);
		
		KeyAdressPair resp = pagAtual.getPairAtIndexOf(0);
	...
	private KeyAdressPair stealBiggest(long pageAdress) throws Exception {
		//retira o par chave/endereço de maior chave
		
		byte[] baPag = new byte[(18 * ordem) - 8];
		indexFile.seek(pageAdress);
		indexFile.read(baPag);
		Pagina pagAtual = new Pagina(baPag, this.ordem);
		
		KeyAdressPair aux = pagAtual.getPairAtIndexOf(pagAtual.getN() - 1);
	...
	private int hasFilledSibling(byte[] baPagAtual, long endPagAtual, long endPagPai) throws Exception {
		byte[] baPai = new byte[(18 * ordem) - 8];
		byte[] baIrma1 = new byte[(18 * ordem) - 8];
		byte[] baIrma2 = new byte[(18 * ordem) - 8];
		Pagina paginaIrma;
		indexFile.seek(endPagPai);
	...
	private void removerOrdenado(short key, Pagina pagAtual) throws Exception {
		for(int i=0; i<pagAtual.getN(); i++) {
	...
	private KeyAdressPair getAntecessor(Pagina pagAtual, short pairIndex) throws Exception {
		byte[] ba = new byte[(18 * ordem) - 8];
		indexFile.seek(pagAtual.getSonAtIndexOf(pairIndex));
		indexFile.read(ba);
		pagAtual = new Pagina(ba, this.ordem);
		
		while(!pagAtual.isLeaf()) {
			long adress = pagAtual.getSonAtIndexOf(pagAtual.getN());
			//Recebe o endereço do último filho
			
			indexFile.seek(adress);
			indexFile.read(ba);
			pagAtual = new Pagina(ba, this.ordem);
	...
	private long[] getEndAntecessor(Pagina pagAtual, long iniAdress, short pairIndex) throws Exception {
		byte[] ba = new byte[(18 * ordem) - 8];
		long parentAdress;
		...
		if(!hasLeafChildren(pagAtual)) {
			...
			parentAdress = pagAtual.getSonAtIndexOf(pairIndex);
			indexFile.seek(parentAdress);
			indexFile.read(ba);
			pagAtual = new Pagina(ba, this.ordem);
		}
		else {
			parentAdress = iniAdress;
		...
		while(!hasLeafChildren(pagAtual)) {
			parentAdress = pagAtual.getSonAtIndexOf(pagAtual.getN());
			//Recebe o endereço do último filho
		...
		long leafAdress = pagAtual.getSonAtIndexOf(pagAtual.getN());
	...
	private boolean hasLeafChildren(Pagina pagAtual) throws Exception {
		...
		long firstSonAdress = pagAtual.getSonAtIndexOf(0);
		byte[] ba = new byte[(18 * ordem) - 8];
		indexFile.seek(firstSonAdress);
		indexFile.read(ba);
