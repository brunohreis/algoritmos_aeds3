...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
		...
		btree = new ArvoreB("btree.index", (short)10);
		...
		while(!aux.equalsIgnoreCase("FIM")) {
			...
			if(aux.equalsIgnoreCase("create")) {
				...
				if(answer == 1) {
					...
					ArrayList<Short> array = multilista.search(country); 
					int i;
					...
					ArrayList<Short> array = multilista.search(year);
					int i;
					for(i=0; i<array.size(); i++) {
						movie = read(array.get(i));
						if(movie != null) {
					...
					scanner.nextLine();
					String country = scanner.nextLine();
					ArrayList<Short> arrayYear = multilista.search(year);
					ArrayList<Short> arrayCountry = multilista.search(country); 
					ArrayList<Short> array = intercessao(arrayYear, arrayCountry);
				...
				System.out.print("ID: ");
				movie = delete(scanner.nextShort());
				if(movie != null) {
				...
				System.out.print("ID: ");
				movie.setId(scanner.nextShort()); //Como se trata de uma atualização, o id do registro é definido pelo usuário, já que ele se trata do id do filme que será substituído
	...
	private static ArrayList<Short> intercessao(ArrayList<Short> arrayYear, ArrayList<Short> arrayCountry) {
		ArrayList<Short> resp = new ArrayList<Short>();
	...
	private static void cargaBase(String arquivoCSV, short qtdReg) throws Exception {
	...
	private static Movie read(short id) throws Exception {
		try {
			Movie aux;
	...
	private static short create(Movie movie) throws Exception {
		byte[] b;
		...
		try {
			...
			movie.setId((short)(file.readShort() + 1));
			file.seek(0);
			file.writeShort(movie.getId());
		  ...
		  try {
			  ...
			  if(adress != -1) {
				  ...
				  if(bNew.length <= ba.length) {
					  ...
					  btree.updateAdress(newMovie.getId(), file.length());
					  file.seek(adress + 4);
	...
	private static Movie delete(short id) throws Exception {
...
//			  }
	...
	private static void sort() throws Exception{
        ...
        tempFile1.seek(0);
        tempFile2.seek(0);
        blocosArq1 = tempFile1.readShort();
        blocosArq2 = tempFile2.readShort();
        ...
        while(blocosArq1 > 1 || blocosArq2 > 1) { //A intercalação é feita até que só exista um bloco nos dois arquivos
        	...
        	destino1.writeShort(0);
        	destino2.writeShort(0);
        	...
        	for(int k=0; k<blocosArq1; k++) {
            	...
            	short aux = destino_grupo.readShort();
            	destino_grupo.seek(0);
            	destino_grupo.writeShort(aux + 1);
        	...
        	destino1.seek(0);
        	destino2.seek(0);
        	blocosArq1 = destino1.readShort();
        	blocosArq2 = destino2.readShort();
        	tamanho_grupo *= 2;
        ...
        while(pos < file.length()) {
        	...
        	Movie movie = new Movie();
        	movie.fromByteArray(ba);
        	btree.updateAdress(movie.getId(), pos);
	...
	private static void escreverBlocosOrdenados(RandomAccessFile tempFile1, RandomAccessFile tempFile2){
		...
		try {
			...
			tempFile1.seek(0);
			tempFile2.seek(0);
			tempFile1.writeShort(0);
			tempFile2.writeShort(0);
            ...
            while(file.getFilePointer() < file.length()) {
            	...
            	destino.seek(0);
    			blocos = destino.readShort();
    			destino.seek(0);
    			destino.writeShort(blocos + 1);
	...
	private static void lineToObject(String linha, Movie movie) {
		...
		movie.setYear(Short.parseShort(data[2].stripIndent()));
	...
	private static void compress(String fileName) throws IOException {
		...
		ArrayList<Short> outAL = new ArrayList<Short>();
		...
		while(cont < fileLength) {
			...
			while(cont < fileLength) {
				byteSeq.add(fileBa[cont]);
				found = false;
				for(short i=0; i<dicionario.size(); i++) {
					if(dicionario.get(i).equals(byteSeq)) {
						found = true;
						wordIndex = i;
						break;
		...
		for(int i=0; i<outAL.size(); i++) {
			short num = outAL.get(i);
			byte[] baAux = ByteBuffer.allocate(2).putShort(num).array();
	...
	private static void decompress(String fileName) throws Exception {
		...
		if(compFile.length() > 0) {
			...
			short[] fileArray = new short[fileLength/2];
			for(int i=0; i<fileArray.length; i++) {
				byte[] baAux = new byte[2];
				for(int j=0; j<2; j++) {
					baAux[j] = fileBa[(2*i) + j];
				}
				fileArray[i] = ByteBuffer.wrap(baAux).getShort();
			}
			//O arquivo compactado é convertido em um array de índices do tipo short
			...
			file.seek(0);
			System.out.println("Último id: " + file.readShort());
		}
		else {
	...
	private static void casamento(String pattern) throws Exception {
		...
		file.seek(0);
		short lastId = file.readShort();
		if(lastId > 0) {
			for(short i=1; i<=lastId; i++) {
				Movie aux = read(i);
				if(aux != null) {
	...
	public static String vigenereEncrypt(String text, String key) {
		...
		for(int i=0; i < text.length(); i++) {
			encryptedTxt += (char)(((int)text.charAt(i) + (int)key.charAt(i) + 256) % 256);
	...
	public static String vigenereDecrypt(String text, String key) {
		...
		//A chave passada como parâmetro é transformada em uma string com repetições da chave original, para que tenha o mesmo tamanho do texto.
		for(int i=0; i < text.length(); i++) {
			decryptedTxt += (char)(((int)text.charAt(i) - (int)key.charAt(i) + 256) % 256);
