...
public class Multilista {
	...
	public void inserir(short id, String country, short year) throws IOException {
		dirYear.seek(0);
		dirCountry.seek(0);
		...
		while(posDir < dirYear.length()) {
			...
			if(rda.getYear() == year) {
				...
				rda.setAmount((short)(rda.getAmount() + 1));
				dirYear.seek(posDir);
		...
		if(!found) {
			...
			dirYear.write(new RegistroDiretorioAno(year, (short)1, list.length()).getBa());
		...
		while(posDir < dirCountry.length()) {
			...
			if(rdp.getCountry().equals(country)) {
				...
				rdp.setAmount((short)(rdp.getAmount() + 1));
		...
		if(!found) {
			...
			byte[] aux = new RegistroDiretorioPais(country, (short)1, list.length()).getBa() ;
	...
	public boolean remover(short id, String country, short year) throws Exception{
	...
	public ArrayList<Short> search(short year) throws IOException {
		ArrayList<Short> array = new ArrayList<Short>(0);
	...
	public ArrayList<Short> search(String country) throws Exception{
		ArrayList<Short> array = new ArrayList<Short>(0);
		long pos;
		byte[] ba;
		int length;
